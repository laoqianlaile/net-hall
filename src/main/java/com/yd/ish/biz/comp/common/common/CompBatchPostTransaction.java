 ...
 */
@Component("CompBatchPostTransaction")
public class CompBatchPostTransaction extends AbsCompBatchTransaction {
...
//    private static final String FILE_SEPARATORS = ReadProperty.getString("file_separators");
    ...
    private static final String FILE_SEPARATORS1 = ReadProperty.getString("file_separators");
    /**
     * 取错误码
     */
...
//        String wjmc = operid + "01_" + instance + new Date().getTime();
        String wjmc = format.format(new Date()) + "/batch/" + new Date().getTime() + ".txt";
        // 文件全路径及文件名
...
//                    fileList = FileUtils.readLines(errfile, ENCODING);
                    fileList = FileUtils.readLines(errfile, "GBK");
                    String strLine;
                    ...
                    if (fileList != null) {
                        for (int i = 0; i < fileList.size(); i++) {
                            ...
                            bean = new BatchExceptionBean(
                                    i + 1,
                                    strLine.split(FILE_SEPARATORS1)[0],//2020-05-21修改
                                    strLine.split(FILE_SEPARATORS1)[1],
                                    strLine.split(FILE_SEPARATORS1)[2],
                                    "0",
                                    strLine.split(FILE_SEPARATORS1)[0],//此处越界 2020-05-21 strLine.split(FILE_SEPARATORS1)[3].split("=")[0]
                                    batchDataListId,
                                    strLine.split(FILE_SEPARATORS1)[1],//此处越界 2020-05-21 strLine.split(FILE_SEPARATORS1)[3].split("=")[1]
            ...
            if (errlist != null && errlist.size() > 0) {
                TransBatchException e = new TransBatchException(errlist, "1");
                e.commit();
                throw e;
            }
