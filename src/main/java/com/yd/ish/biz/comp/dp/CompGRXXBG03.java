 ...
 * @note
 */
@Component("CompGRXXBG03")
public class CompGRXXBG03 extends BaseComp {

	private static final Logger logger = LoggerFactory.getLogger(CompGRXXBG03.class);
	/** 上传文件本地路径 */
	...
	public int execute() {
		...
		String operid = getString("_OPERID");// 操作员id
		/* 入口参数赋值结束 */
		...
		try {
			...
			if (list != null && list.size() > 0) {
				...
				// 第一行写入列名
				fw.write("grzh" + file_separators + "bghzc" + file_separators + "bghzw" + file_separators + "bghzy"
						+ file_separators + "bghxl" + file_separators + "bghsjhm" + file_separators + "bghjtzz");
				fw.write(System.lineSeparator());
				// 循环写入文件信息
				for (int i = 0; i < list.size(); i++) {
					fw.write(list.get(i).getUnitaccnum1() + file_separators + list.get(i).getUnitaccname2()
							+ file_separators + list.get(i).getUnitaccnum2() + file_separators + list.get(i).getBegym()
							+ file_separators + list.get(i).getZip() + file_separators + list.get(i).getAccnum2()
							+ file_separators + list.get(i).getXmqp());
					if (i < list.size() - 1) {
						fw.write(System.lineSeparator());
					}
					if (i % 1000 == 0)
		...
		logger.info("[+]调用接口提交个人信息变更开始");

		// 调用接口提交个人信息变更
		XmlResObj data = super.sendExternal("BSP_DP_DWXGRXXGB_01");
		...
		if (ERROR_PLYC.equals(head.getParticular_code())) {
			...
			String errfilename = "grxxbg" + ".err";// 本地测试自定义批量错误文件名 TODO
					...
					if (fileList != null) {
						for (int i = 0; i < fileList.size(); i++) {
							...
							bean = new BatchExceptionBean(i + 1, strLine.split(file_separators)[0],
									...
									strLine.split(file_separators)[3].split("=")[0], "datalist1",
