 ...
 * @note
 */
@Component("CompFYLPXX04")
public class CompFYLPXX04 extends BaseComp {

    private static final Logger logger = LoggerFactory.getLogger(CompFYLPXX04.class);
    /**
     * 数据分割符
     */
    ...
    public int execute() {
        ...
        logger.info("[+]调用接口获取幢信息，存临时表开始");
        // 调用接口获取幢信息
        XmlResObj data = super.sendExternal("BSP_LN_ZXXCX_04");
                ...
                try {
                    ...
                    while ((tmp = br.readLine()) != null) {
                        if (first) {
                            ...
                            dp077.setDpbusitype("04");// 业务类型，01-开发商信息；02-楼盘信息；03-房源楼盘信息；04-幢信息
                            dp077.setInstance(instance);// 实例号
                            dp077.setUnitaccnum1(tmpdata.get("jzzh").toString());// 建筑幢号
                            dp077.setUnitaccnum2(tmpdata.get("gazh").toString());// 公安幢号
                            dp077.setUnitaccname1(tmpdata.get("zflx").toString());// 住房类型
                            dp077.setUnitaccname2(tmpdata.get("sfyzx").toString());// 是否有装修
                            dp077.setAccname1(tmpdata.get("zxbz").toString());// 装修标准
                            dp077.setAccnum1(tmpdata.get("zcs").toString());// 总层数
                            dp077.setAccnum2(tmpdata.get("dqcs").toString());// 当前层数
                            list.add(dp077);
                            // 累计循环一定条数批量提交入库一次
