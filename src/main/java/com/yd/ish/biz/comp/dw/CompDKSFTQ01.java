 ...
 * @note
 */
@Component("CompDKSFTQ01")
public class CompDKSFTQ01 extends BaseComp{

	private static final Logger logger = LoggerFactory.getLogger(CompDKSFTQ01.class);
	@Autowired
	...
	public int execute() {
		...
		if(flag) {
			...
			// 调用接口计算提取金额
			XmlResObj data = super.sendExternal("BSP_DW_JSDKSFTQJE_01");
			...
			logger.info("[-]调用接口计算提取金额结束");
			/*出口参数赋值开始*/
			setOutParam("ktqje", ktqje);//可提取金额
			/*出口参数赋值结束*/
		}else{
			logger.error("计算提取金额失败,获取到的借款合同编号："+ jkhtbh + ",不在可选范围内");
			throw new TransSingleException("计算提取金额失败，请确认后重新提交。");
		}
    	return 0;
   }
