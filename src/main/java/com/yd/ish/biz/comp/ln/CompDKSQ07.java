 ...
 * @note
 */
@Component("CompDKSQ07")
public class CompDKSQ07 extends BaseComp{

	private static final Logger logger = LoggerFactory.getLogger(CompDKSQ07.class);
	/** 数据分割符 */
	...
	public int execute() {
		...
		logger.info("[+]调用接口获取全部支行信息，存临时表开始");
		// 调用接口获取全部支行信息
		XmlResObj data = super.sendExternal("BSP_LN_GETZHIHANG_01");
				...
				try {
					...
					while ((tmp = br.readLine()) != null) {
						if (first) {
							...
							dp077.setDpbusitype("00");// 业务类型，00-查询
							dp077.setInstance(instance);// 实例号
							dp077.setUnitaccnum1(tmpdata.get("zhbh").toString());// 支行编号
							dp077.setUnitaccname1(tmpdata.get("zhmc").toString());// 支行名称
							list.add(dp077);
							// 累计循环一定条数批量提交入库一次
					...
					logger.info("[-]调用接口获取全部支行信息，存临时表结束");
