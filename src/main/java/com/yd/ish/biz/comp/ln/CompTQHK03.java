 ...
 * @brief 获取公积金还款人信息
 * @author 柏慧敏
 * @version 0.1 2018年7月3日 柏慧敏创建
 * @note
 */
@Component("CompTQHK03")
public class CompTQHK03 extends BaseComp{

	private static final Logger logger = LoggerFactory.getLogger(CompTQHK03.class);
	/** 数据分割符 */
	...
	public int execute() {
		...
		// 若flag为true，调用接口获取还款人信息
		if(flag) {
			logger.info("[+]调用接口根据借款合同编号获取还款人信息，存临时表开始");
			// 调用接口根据借款合同编号获取还款人信息
			XmlResObj data = super.sendExternal("BSP_LN_TQHK_03");
					...
					try {
						...
						while ((tmp = br.readLine()) != null) {
							if (first) {
								...
								dp077.setAccnum1(tmpdata.get("gjjzh") == null ? "" : tmpdata.get("gjjzh").toString());// 公积金账号
								...
								dp077.setAmt1(tmpdata.get("kyye") == null ? new BigDecimal("0") : new BigDecimal(tmpdata.get("kyye").toString()));// 可用余额
								dp077.setHyzk(tmpdata.get("jkrlx") == null ? "" : tmpdata.get("jkrlx").toString());// 借款人类型
								list.add(dp077);
								// 累计循环一定条数批量提交入库一次
						...
						logger.info("[-]调用接口根据借款合同编号获取还款人信息，存临时表结束");
			...
			throw new TransSingleException("获取信息失败，请确认后重新录入");
		}
    	/*出口参数赋值开始*/
