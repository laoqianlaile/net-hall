 ...
 * @author 柏慧敏
 * @version 0.1 2018年9月6日 柏慧敏创建
 * 			0.2 2018年10月30日  柏慧敏修改  可以按照核心要求的编码格式写上传文件
 * @note
 */
@Component("CompDWTJ02")
public class CompDWTJ02 extends BaseComp{

	private static final Logger logger = LoggerFactory.getLogger(CompDWTJ02.class);
	/** 上传文件本地路径 */
	...
	public int execute() {
		...
		try {
			...
			if (list != null && list.size() > 0) {
				...
				// 第一行写入列名
				fw.write("grzh" + file_separators + "xingming" + file_separators + "yjce" + file_separators + "grzhye"
						+ file_separators + "tjje" + file_separators + "tjyy");
				fw.write(System.lineSeparator());
				// 循环写入文件信息
				for (int i = 0; i < list.size(); i++) {
					fw.write(list.get(i).getUnitaccnum1() + file_separators + list.get(i).getAccname1() + file_separators
							+ list.get(i).getBasenum() + file_separators + list.get(i).getAmt1() + file_separators
							+ list.get(i).getAmt2() + file_separators + list.get(i).getReason());
					if (i < list.size() - 1) {
						fw.write(System.lineSeparator());
					}
					if (i % 1000 == 0)
		...
		logger.info("[+]调用接口提交退缴信息开始");

		// 调用接口提交退缴信息
		XmlResObj data = super.sendExternal("BSP_DP_DWTJXX_01");
		...
		if (ERROR_PLYC.equals(head.getParticular_code())) {
			...
			String errfilename = "tjxxmessage" + ".err";//本地测试自定义批量错误文件名 TODO
