 ...
 * @note
 */
@Component("CompFYLPXX02")
public class CompFYLPXX02 extends BaseComp {

	private static final Logger logger = LoggerFactory.getLogger(CompFYLPXX02.class);
	/** 数据分割符 */
	...
	public int execute() {
		...
		// 根据实例号和类型删除当前楼盘数据
		dp077.setDpbusitype("02");
		dp077service.deleteByCause(dp077);
		logger.info("[+]调用接口根据开发商获取对应楼盘信息，存临时表开始");
		// 调用接口根据开发商获取对应楼盘信息
		XmlResObj data = super.sendExternal("BSP_LN_CXLPXX_03");
				...
				try {
					...
					while ((tmp = br.readLine()) != null) {
						if (first) {
							...
							dp077.setDpbusitype("02");// 业务类型，01-开发商信息；02-楼盘信息；03-房源楼盘信息
							dp077.setInstance(instance);// 实例号
							dp077.setUnitaccnum2(tmpdata.get("lpbh").toString());// 楼盘编号
