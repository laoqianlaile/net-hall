 ...
 * @brief 提交开发商项目维护信息
 * @author 柏慧敏
 * @version V0.1 柏慧敏 2019-06-17 长春 新建
 * @note
 */
@Component("CompXMWH01")
public class CompXMWH01 extends BaseComp{

	private static final Logger logger = LoggerFactory.getLogger(CompXMWH01.class);
	 /**
	  * 上传文件本地路径
	  */
	...
	public int execute() {
		...
		// 提交开发商项目维护信息
		XmlResObj data = super.sendExternal("BSP_LN_TJWHXX_01");
					...
					if (errfile != null && errfile.exists()) {
						...
						if (ldfile.equals(filenames[i])) {
							try {
								...
								e.printStackTrace();
								logger.error("文件读取失败,文件名："+ errfile.getPath()+"错误信息："+ e.getMessage(), e);
