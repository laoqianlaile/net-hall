 ...
 * @note
 */
@Component("CompFYLPXX01")
public class CompFYLPXX01 extends BaseComp {

	private static final Logger logger = LoggerFactory.getLogger(CompFYLPXX01.class);
	/** 数据分割符 */
	...
	public int execute() {
		...
		dp077.setDpbusitype("01");
		dp077service.deleteByCause(dp077);

		logger.info("[+]调用接口获取全部开发商信息，存临时表开始");
		// 调用接口获取全部开发商信息
		XmlResObj data = super.sendExternal("BSP_LN_KFSXXCX_02");
				...
				try {
					...
					while ((tmp = br.readLine()) != null) {
						if (first) {
							...
							dp077.setDpbusitype("01");// 业务类型，01-开发商信息；02-楼盘信息；03-房源楼盘信息
							dp077.setInstance(instance);// 实例号
							dp077.setUnitaccnum1(tmpdata.get("kfsbh").toString());// 开发商编号
