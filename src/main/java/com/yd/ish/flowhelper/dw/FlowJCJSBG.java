...
@Component
public class FlowJCJSBG implements FlowHelperI {
    private static final Logger logger = LoggerFactory.getLogger(FlowJCJSBG.class);
    ...
    public boolean in(String stepid, String type, MainContext mainContext) {
        ...
        if(stepid.equals("step2")){
            ...
            //获取打印模板
            String poolkey = yDVoucherUtil.savePdfVoucher(mainContext.getDataPool(), "JCJSBGDY");
    ...
    public boolean out(String stepid, MainContext mainContext) {
        if(stepid.equals(STEP_STEP1)){
            TransEngine.getInstance().execute("TranJCJSBG1", mainContext);
        }
        if(stepid.equals("step2")){
            TransEngine.getInstance().execute("TranJCJSBG2", mainContext);
        }
        return true;
    }
