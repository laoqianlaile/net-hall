 ...
 */
@Service
public class PdfDWBJPZ01 implements IYDVoucher {
	...
	private static final Logger logger = LoggerFactory.getLogger(PdfDWBJPZ01.class);
	/** 启用凭证系统标记 **/
	...
	public File generateVoucher(DataPool pool, int cs) {
		...
		if (!"1".equals(pzsystemFlag)) {
			...
			// 终止年月
			excelEngine.setData("zzny", pool.getString("jzny"));
			// 收款账号
			excelEngine.setData("skzh", pool.getString("skyhzh"));
			// 收款银行
			excelEngine.setData("skyh", pool.getString("skyhmc"));
			// 应缴金额
			excelEngine.setData("yjje", MoneyUtil.addComma(pool.getString("sjje")));
			// 大写应缴金额
			excelEngine.setData("dxyjje", MoneyUtil.numberToChineseRMB(pool.getString("sjje").replaceAll(",", "")));
			// 汇缴人数
			excelEngine.setData("jcrs", pool.getString("bjrs"));
			// 实缴金额
			excelEngine.setData("sjje", MoneyUtil.addComma(pool.getString("sjje")));
			// 暂存户转出金额
			excelEngine.setData("zchzcje", MoneyUtil.addComma(pool.getString("zchzcje").replaceAll(",", "")));
			// 经办机构
