 ...
 */
@Service
public class PdfDWBJPZ02 implements IYDVoucher {
	...
	private static final Logger logger = LoggerFactory.getLogger(PdfDWBJPZ02.class);
	/** 启用凭证系统标记 **/
	...
	public File generateVoucher(DataPool pool, int cs) {
		...
		if (!"1".equals(pzsystemFlag)) {
			...
			// 入账日期
			excelEngine.setData("rzrq", pool.get("rzrq"));
			// 单位账号
			...
			// 结束年月
			excelEngine.setData("jsny", pool.getString("jzny"));
			// 缴存人数
			excelEngine.setData("jcrs", pool.getString("bjrs"));
			// 单位汇缴金额
			...
			// 入账金额
			excelEngine.setData("rzje", MoneyUtil.addComma(pool.getString("sjje").replaceAll(",", "")));
			// 大写入账金额
			excelEngine.setData("rzjedx", MoneyUtil.numberToChineseRMB(pool.getString("sjje").replaceAll(",", "")));
			String jclx = pool.getString("bjlx");
			String jkfs = pool.getString("jkfs");
			String jclxval = paramConfigImp.getVal("bsp.dp.sptype." + jclx);
