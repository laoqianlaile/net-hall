 ...
 */
@Service
public class PdfGRFCPZ01 implements IYDVoucher {

	@Autowired
	ParamConfigImp paramConfigImp;
	@Autowired
	FlowAttachmentImp flowAttachmentImp;
	private static final Logger logger = LoggerFactory.getLogger(PdfGRFCPZ01.class);
	/** 启用凭证系统标记 **/
	...
	public File generateVoucher(DataPool pool, int cs) {
		...
		if (!"1".equals(pzsystemFlag)) {
			...
			excelEngine.setData("transdate", pool.getString("transdate"));
			...
			// 总线中批量数据id
			String _ISH_PRINTJSONDATA = IshConstants._ISH_PRINTJSONDATA;
			// 若总线中有打印批量数据，获取总线中的批量数据（补打）
			if (pool.containsKey(_ISH_PRINTJSONDATA) && !"".equals(_ISH_PRINTJSONDATA)) {
				printJsonData = pool.getString(_ISH_PRINTJSONDATA);
			}
			// 若总线中没有打印需要的批量数据，取临时表数据，放入总线中（首次打印）
				...
				// 根据实例号在临时表中查询个人封存信息
				List<DP077> grfclist = dp077service.selectBySlh(Integer.parseInt(instance));
				// 循环查询结果，打印批量数据list赋值
				if (grfclist != null && grfclist.size() > 0) {
					for (int i = 0; i < grfclist.size(); i++) {
						HashMap<String, Object> map = new HashMap<String, Object>();
						DP077 dp077 = grfclist.get(i);
			...
			while (iterator.hasNext()) {
				...
				list.add(
						new String[] { index.toString(), "个人封存", object.getString("grzh"), object.getString("xingming"),
								_zjlx, object.getString("zjhm"), MoneyUtil.addComma(object.getString("grjcjs")) });
			}
			excelEngine.setData("bgrs", index);// 变更人数
