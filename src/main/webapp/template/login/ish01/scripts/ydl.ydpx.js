...
(function(ydl, $, undefined) {
	...
	ydl.init.dataList = function(options, method) {
			...
			case 'init':
				...
				if (gOptions.dialogEdit && (gOptions.allowUpdate || gOptions.allowInsert)) {
						...
						$.each(gOptions.columns, function(index, col) {
							if (col.colType == COLUMN_EDITABLE) {
								...
								editorFormHtml += '<td><label for="' + colId + '">' + col.desc + '：</label></td><td>' + (col.dict ?
									'<select name="' + col.id + '" id="' + colId + '"' + (col.required ? ' required="required"' : '') +
					...
					$table.on('click', 'tr:gt(0)', function(event) {
						if (!$(this).hasClass('ui-widget-header') && gOptions.dialogEdit && !(event.target.tagName == 'INPUT' && event.target.type == 'checkbox')) {
							var $tr = $(this);
				...
				$table.pagebar({
					...
					buttons: {
						...
						'delete': {
							...
							click: function() {
								if (!buttonClickFunc || buttonClickFunc('delete') !== false) {
									var $checkboxes = $table.find('td:nth-child(1) :checkbox:visible:checked');
									if ($checkboxes.length == 0) alert('请先选择要删除的记录。');
									else if (confirm('请确认是否要删除选择的 ' + $checkboxes.length + ' 条记录？') && deleteData()) alert('删除成功！');
								}
							}
						},
						'save': {
								...
								if (!buttonClickFunc || buttonClickFunc('import') !== false) {
									ydl.dialog.open(ydl.contexPath + '/platform/listUploadInput.jsp', [
										...
										if (data && data.returnCode == 0 && data.data) {
											ydl.init.dataList($.extend(true, {}, data.data), 'update');
											//回调changed函数
						...
						'export': {
							...
							text: '导出',
							click: function() {
								if (!buttonClickFunc || buttonClickFunc('export') !== false) {
									var paras = [
										//{name: '$page', value: ydl.common.parameters['$page']},
										{name: '$page', value: poolSelect['$page']},
				...
				showButtons();
				//$table.next('.page-buttons').css('width', $table.css('width'));
				//回车添加新记录
		...
		function showButtons() {
			...
			if (gOptions.allowInsert || gOptions.allowUpdate) $('#list_' + tableId + '_buttons_save').removeClass('hide');
		...
		//待添加from: 从哪添加记录，'0'表示新增，'1'表示删除后补填当页数据）
		function insertRow(isOriginal, data, index) {
			var ssss = ydl.getMember(data)+'\n';
			!isOriginal && $table.find('tr._datalist-tip').remove();
			var gOptions = ydl.data.dataList[tableId];
			//是否为新增行
			var isAdd = index === undefined;
			if (isAdd) index = table.tBodies[0].rows.length;
			var tr = table.insertRow(index);
			if (isAdd && gOptions.dialogEdit) tr.style.cursor = 'pointer';
			//第一列，复选框和行标记
			//var td = tr.insertCell(0);
			//if (!gOptions.allowDelete && !gOptions.allowInsert) td.className = 'hide';
			/**td.innerHTML = '<td><input type="checkbox" class="_datalist_delete_checkbox' + (isOriginal && !gOptions.allowDelete ? ' hide' : '') + '" />' +
			 '<input type="hidden" name="dynamicTable_flag" value="' + (isOriginal ? FLAG_ORIGINAL : FLAG_NEW) + '" /></td>';
			 */
			$(tr).append('<td ' + (!gOptions.allowDelete && !gOptions.allowInsert?'class="hide"':'') + '><input type="checkbox" class="_datalist_delete_checkbox' + (isOriginal && !gOptions.allowDelete ? ' hide' : '') + '" />' +
				'<input type="hidden" name="dynamicTable_flag" value="' + (isOriginal ? FLAG_ORIGINAL : FLAG_NEW) + '" /></td>');
			if(gOptions.showRowId) $(tr).append('<td class="code">'+(isAdd ? '+' : index + gOptions.pageSize * ((gOptions.currentPage > 0 ? gOptions.currentPage : 1) - 1))+'</td>');
			//var colNum = 1;
			//行号列
			/**
			...
			td = tr.insertCell(1);
			td.className = 'code';
			td.innerHTML = isAdd ? '+' : index + gOptions.pageSize * ((gOptions.currentPage > 0 ? gOptions.currentPage : 1) - 1);
			colNum++;
		}
			 */
			//数据列
			for (var colIndex = 0, colCount = gOptions.columns.length; colIndex < colCount; colIndex++) {
				...
				var fieldId = tableId + '_' + columnId + '_' + index;
				//单元格样式
				var colStyle = '';
				...
				//可编辑
				if (gColumn.colType == COLUMN_EDITABLE && (gOptions.allowUpdate || gOptions.allowInsert && isAdd)) {

					if (gOptions.dialogEdit) {
						//对话框编辑，输出input:hidden + span
						//if (colStyle) td.className = colStyle;
						$(tr).append('<td class="'+(colStyle?colStyle:'')+' ' +(gColumn.hidden?'hide':'' ) + '" ><input type="hidden" id="' + fieldId + '" name="' + columnId + '" value="' + colData + '" />' +
							'<span>' + colDataText + '</span></td>');
					}
					else {
						//有字典，输出select
						if (gColumn.dict) {
							...
							$(tr).append('<td class="'+(colStyle?colStyle:'')+' ' +(gColumn.hidden?'hide':'' ) + '"><select id="' + fieldId + '" name="' + columnId + '"' +
								(gColumn.required ? 'required="required"' : '') + '>' + gOptions.dictCache[columnId] + '</select></td>');
							//td.innerHTML = ;
							isOriginal && ydl.selectByValue(document.getElementById(fieldId), colData);
						}
						//无字典，输出input:text
						else {
							$(tr).append('<td class="'+(colStyle?colStyle:'')+' ' +(gColumn.hidden?'hide':'' ) + '" ><input type="text" id="' + fieldId + '" name="' + 	columnId + '" value="' + colData + '"' +
								...
								(gColumn.required ? ' required="required"' : '') + (colStyle ? ' class="' + colStyle + '"' : '') + ' /></td>');
							/**
				...
				//不可编辑，只读输出span，条件字段输出input:hidden + span
					...
					//if (colStyle) td.className = colStyle;
					$(tr).append('<td class="'+ colStyle +' ' +(gColumn.hidden?'hide':'' ) + (gColumn.dict?' title='+ colData : '') +'" >'+(gColumn.colType == COLUMN_TERM || gOptions.allowDelete ?
						'<input type="hidden" id="' + fieldId + '" name="' + columnId +	'" value="' + colData + '" />' : '') +
						'<span>' + colDataText + '</span></td>');
					/**
					 td.innerHTML = (gColumn.colType == COLUMN_TERM || gOptions.allowDelete ?
					 '<input type="hidden" id="' + fieldId + '" name="' + columnId +	'" value="' + colData + '" />' : '') +
		...
		function deleteData() {
				...
				if (this.value == FLAG_ORIGINAL) {	//只提交原数据删除的情况
					$(this).closest('tr').find(':input[name]').each(function() {
						postData.push({
							...
							value: this.name == 'dynamicTable_flag' ? FLAG_DELETED : (this.value == '' && !ischar[this.name] ? 'null' : this.value)
						});
					});
				}
			...
			$('table:not(.datalist,.datalist-editor-dialog) :input[name],div>:input[name]').each(function() {
				postData.push({
					name: this.name,
					value: this.value
				});
			...
			if (!isAllNew) {
				ydl.ajax(ydl.contexPath + '/dynamictable', postData, function (returnData) {
					//删除数据和列表行
					var flagnum = $flag.length;
					//删除新增的记录数
					var newnum = 0;
					$flag.each(function() {
						//gOptions.data.splice(lo($table.find('._datalist_delete_checkbox').next().index(this),'orange'), 1);
						if (this.value == FLAG_NEW) newnum++;
						$(this).closest('tr').remove();
					});
					//如果删除后，列表中没有数据，则重新查询当前页
					var rowLength = table.tBodies[0].rows.length;
					if (rowLength == 1) {
						gOptions.data = [];
						saveData(returnData.currentPage || 1);
					}
					else {
						//如果不是最后一页且删除的数据不全为新增
						if (gOptions.currentPage != gOptions.pageCount && !isAllNew) {
							/*--添加删除项的后补数据--*/
							//删除前条数
							var datalength = gOptions.data.length;
							//删除后条数
							var curDatalength = returnData.data.length;
							//补数据条数(如果删除后当前页条数小于删除前当前页条数，补数据条数= 删除后条数-(删除前条数-已删除条数)，否则补数据条数=已删除条数)-删除新增条数
							flagnum = ((curDatalength < datalength) ? (curDatalength + flagnum - datalength) : flagnum) - newnum;
							for (var i = 0 ; i < flagnum; i++) {
								insertRow(true, returnData.data[curDatalength - flagnum + i], rowLength + i);
							}
						}
						//更新全局对象
						...
						if (gOptions.showRowId) {
							var col = 0;
							$('#' + tableId).find('td:nth-child(2)').each(function (index) {
								if ($(this).prev().find('[name=dynamicTable_flag]').val() == FLAG_NEW) col--;
								else this.innerHTML = col + 1 + gOptions.pageSize * ((gOptions.currentPage > 0 ? gOptions.currentPage : 1) - 1);
								col++;
							});
						}
						callback();
			...
			//设置页码栏宽度
			var width = parseInt(table.clientWidth) - 12;
			//$table.next('.page-buttons').css({'max-width': width, 'width': width});
			return success;
		}
		...
		function saveData(pageNo, fromSaveButton) {
						...
						for (colIndex = 0, colCount = gOptions.columns.length; colIndex < colCount; colIndex++) {
							...
							if (gColumn.colType == COLUMN_EDITABLE) {	//只检查可编辑列
								if (gOptions.data[dataIndex][gColumn.id] !== undefined) {
									...
									if (originalData.trim() != '' || currentData.trim() != '') {
			...
			if (!cancelUpdate) {
				...
				var changed = false;
				//$flag.each(function() {
				$table.find('input[name=dynamicTable_flag]').each(function() {
			...
			//同步提交
			var success = false;
			if (($checks.length == 0 && !cancelUpdate) || ($checks.length > 0 && !cancelUpdate) && (cancelUpdate || confirm('请确认是否要删除选择的 ' + $checks.length + ' 条记录？'))) {
				ydl.ajax(ydl.contexPath + "/dynamictable", postData, function(returnData) {
					//更新全局对象
					$.extend(ydl.data.dataList[tableId], {
						data: returnData.data,
				...
				}, {async: false,handleError: function(returnData) {
						reflag(false);
						alert('提示信息：\n\n' + returnData.message)
					}});
			}
			...
			//恢复数据 isChecked,是否取消选中状态 true取消
			function reflag(isChecked){
				var $checkbox = $table.find('._datalist_delete_checkbox:checked');
				if (isChecked) $checkbox.removeAttr('checked');
				//恢复选中checkbox的原状态
				$checkbox.each(function(index, ele) {
					var flag = ele.nextSibling;
					if (flag.value == FLAG_DELETED) flag.value = FLAG_ORIGINAL;	//原数据删除状态 恢复成 原数据状态
					else if (flag.value == FLAG_ABORTED) flag.value = FLAG_NEW;	//新增数据删除 恢复成 新增数据状态
				});
				//恢复修改状态
				$table.find('input[name=dynamicTable_flag][value='+FLAG_MODIFIED+']').val(FLAG_ORIGINAL);
			}
		}
	};
	...
	ydl.init.list = function(listId, paras) {
		...
		function initSort() {
			...
			$list.find('label.sortable').click(function() {
				if ($$('filter_floatdiv')) $('#filter_floatdiv').hide();
				listId = $(this).closest('.container').prop('id');
				listParameter = ydl.data.listParameter[listId];
		...
		function initFilterData() {
			...
			if (!$$('filter_floatdiv')) {
				...
				$('#sort_asc').click(function() {
					listParameter = $('#filter_floatdiv').data('listParameter');
				...
				$('#filter_data_allclear').click(function() {
					...
					$('#'+$('#filter_floatdiv').data('listId')).pagebar(doQuery(1));	//查询并更新
				});
				//清除当前列筛选条件
			...
			function clickIcon($icon, colnum) {
				...
				if ($filterDiv.is(':hidden')) {
					...
					//获取当前列筛选的字段
					listId = $icon.closest('.container').prop('id');
					ydl.data.listParameter[listId].curfilter = $icon.prev().get(0).className.match(/\b_sortname_(.+)\b/)[1];
					$filterDiv.data('listParameter',ydl.data.listParameter[listId]);
					$filterDiv.data('listId',listId);
					$filterDiv.show();
				}
				//隐藏浮动层时
				else $filterDiv.hide();
		...
		function doQuery(pageNo) {
			listId = $('#filter_floatdiv').is(':hidden') ? listId : $('#filter_floatdiv').data('listId');
			//提交前的校验
			...
			ydl.ajax(ydl.contexPath + '/parsepage', $('form:first').serialize() + '&' + $.param(postData), function(data) {
				if (data.indexOf('<table id="' + listId + '"') >= 0) {
					...
					window['list_callback_' + listId] && window['list_callback_' + listId](document.getElementById(listId).rows);
					//列表中第一个输入域设置焦点
					$('#' + listId + ' :input:visible:not(:disabled,[readonly=readonly]):first').focus();
					////设置页码栏宽度
					//var minwidth = $.browser.msie && $.browser.version < 9.0 ? 0 : 2;
					//var width = parseInt($('#'+listId).css('width')) - minwidth;
					//$('#'+listId).next().next('.page-buttons').width(width).css('max-width',width);
				}
				else alert('服务器出错！');
			}, {
				dataType: 'html',
	...
	ydl.ydpxInit = function(initComponents) {
		...
		try {
			...
			try {
				...
				throw '在初始化页面组件时发生错误：' + ydl.error(e);
			}
			...
			try {
				...
				throw '在初始化双敲复核时发生错误：' + ydl.error(e);
			}
			...
			function runFormValidate(form) {
				if (window.form_validate) {
					try {
						//以下针对数字证书签名增加了对方法OnFormSubmit的调用，该方法在SecX_Common.js中  威海现场
						var r = form_validate(form);
						if(r&&OnFormSubmit){
							r = OnFormSubmit();
						}
						return r;
			...
			$('form').submit(function() {
				...
				if ($('input[name=_APPLY]').val() == '1' || ydl.formValidate(this) && runFormValidate(this)) {
					...
					if ($(form).attr('data-ajax') == 'false') {
						...
						//通过Ajax提交表单
						ydl.ajax(form.action, $(form).getInputs(true), function (data, oriData) {
//					ydl.ajax(form.action, $(form).serialize(), function (data, oriData) {
							...
							complete: function () {
								...
								$('button.flow_chehui').prop('disabled', true);
			...
			try {
				...
				throw '在pageOnload函数内部发生错误：' + ydl.error(e);
			}

			if (ydl.localData('operconfig_changeFocus') == 'enter') {
				//回车自动跳转到下一个输入域，直到提交按钮
				$('form').on('keydown', ':input:visible:not(:disabled,[readonly=readonly],[tabindex=-1])', function(event) {
					if (event.which == ydl.common.keys.Enter) {
						var $thisInput = $(this);
			...
			try {
				...
				if (!inputEvents || !inputEvents['focus']) $inputs.eq(0).focus();
			}
			catch (e) {
				ydl.log('将第一个表单输入域设为焦点时出错' + ydl.error(e));
			}
